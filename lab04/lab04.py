...
def reverse_recursive(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    def reverse_recursive_helper(lst,index,ans):
        if index == len(lst)-1:
            ans.append(lst[index])
            return ans
        else:
            ans = reverse_recursive_helper(lst,index+1,ans)
            ans.append(lst[index])
            return ans
    return reverse_recursive_helper(lst,0,[])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_n = make_city('i',lat,lon)
    if distance(city_n,city_a) < distance(city_n,city_b):
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_chars(w1,index1,w2,index2,ans):
        if index2 >= len(w2):
            return ans
        if index1 >= len(w1):
            return add_chars(w1,index1,w2,index2+1,ans+w2[index2])
        if w1[index1] == w2[index2]:
            return add_chars(w1,index1+1,w2,index2+1,ans)
        else:
            return add_chars(w1,index1,w2,index2+1,ans+w2[index2])
    return add_chars(w1,0,w2,0,"")
