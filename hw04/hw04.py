...
def balanced(m):
    ...
    if is_planet(m):
        ...
        #print(total_weight(end(right(m))))
        #print(total_weight(end(left(m))))
...
def replace_leaf(t, find_value, replace_value):
    ...
    new_branches = []
    for b in branches(t):
        new_branches.append(replace_leaf(b,find_value,replace_value))
    if label(t) == find_value and is_leaf(t):
        return [replace_value] + new_branches
    else:
        return [label(t)] + new_branches
...
def has_path(t, phrase):
    ...
    "*** YOUR CODE HERE ***"
    def has_path_helper(t,phrase,now):
        now = now + label(t)
        ans = False
        if now == phrase:
            return True
            ...
            for b in branches(t):
                ans = ans or has_path_helper(b,phrase,now)
        return ans
    return has_path_helper(t,phrase,"")
...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a*t*t + b*t + c
    return interval(min(f(lower_bound(x)),f(upper_bound(x)),f(d)),max(f(lower_bound(x)),f(upper_bound(x)),f(d)))
