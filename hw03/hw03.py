...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def missing_digits_helper(n,last,ans):
        if n == 0:
            return ans
        if last == -1:
            return missing_digits_helper(n//10,n%10,ans)
        else:
            return missing_digits_helper(n//10,n%10,max(ans + last - n%10 -1,ans))


    return missing_digits_helper(n,-1,0)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_change_helper(total,now):
        if total == 0:
            return 1
        if now > total:
            return 0
        return count_change_helper(total-now,now) + count_change_helper(total,now*2)
